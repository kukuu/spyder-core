...
const processNaturalLanguageQuery = (query, readingsData, historicalData) => {
  ...
  if (queryLower.includes('this week') || queryLower.includes('weekly')) {
    ...
    Object.entries(historicalData).forEach(([meterId, readings]) => {
      const weeklyReadings = readings.filter(reading => 
        new Date(reading.timestamp) >= oneWeekAgo
      );
      
      if (weeklyReadings.length > 0) {
        const total = weeklyReadings.reduce((sum, r) => sum + parseFloat(r.reading), 0);
        const avg = total / weeklyReadings.length;
        results.push({
          meterId,
          type: 'weekly',
          total,
  ...
  // Performance queries
  if (queryLower.includes('worst') || queryLower.includes('poor') || queryLower.includes('bad')) {
    Object.entries(readingsData).forEach(([meterId, data]) => {
      const currentReading = parseFloat(data.reading) || 0;
      const consumptionRate = consumptionRates[meterId] || "Normal";
      
      if (consumptionRate.includes('High') || currentReading > 100) {
        results.push({
          meterId,
          type: 'performance',
  ...
  if (queryLower.includes('best') || queryLower.includes('good') || queryLower.includes('excellent')) {
    Object.entries(readingsData).forEach(([meterId, data]) => {
      const currentReading = parseFloat(data.reading) || 0;
      const consumptionRate = consumptionRates[meterId] || "Normal";
      
      if (consumptionRate.includes('Low') || currentReading < 50) {
        results.push({
          meterId,
          type: 'performance',
          reading: currentReading,
  ...
  // Cost queries
  if (queryLower.includes('expensive') || queryLower.includes('high cost')) {
    Object.entries(readingsData).forEach(([meterId, data]) => {
      const currentReading = parseFloat(data.reading) || 0;
  ...
  // If no specific matches, return all meters
  if (results.length === 0) {
    Object.entries(readingsData).forEach(([meterId, data]) => {
      const currentReading = parseFloat(data.reading) || 0;
      results.push({
...
const QueryResults = memo(({ results, onClose }) => {
  if (!results || results.length === 0) return null;

  return (
    <div className="query-results-modal">
        ...
        <div className="query-results-body">
          {results.map((result, index) => (
            <div key={index} className="query-result-item">
              <h4>Meter: {result.meterId}</h4>
              <p>Type: {result.type}</p>
              {result.reading && <p>Current Reading: {result.reading} kWh</p>}
              {result.consumptionRate && <p>Consumption Rate: {result.consumptionRate}</p>}
              {result.status && <p>Status: {result.status}</p>}
              {result.cost && <p>Cost: £{result.cost}</p>}
              {result.supplier && <p>Supplier: {result.supplier}</p>}
              {result.average && <p>Weekly Average: {result.average.toFixed(2)} kWh</p>}
              {result.total && <p>Weekly Total: {result.total.toFixed(2)} kWh</p>}
            </div>
          ))}
        </div>
      </div>
...
function AppWithPayPalProvider() {
  ...
  const handleQueryResultsClose = useCallback(() => {
  ...
  const handleNaturalLanguageQuery = () => {
    if (naturalLanguageQuery.trim()) {
      const results = processNaturalLanguageQuery(naturalLanguageQuery, readings, historicalData);
  ...
  const renderQueryResults = () => {
    if (!showQueryResults) return null;

    return (
      <QueryResults 
        results={queryResults}
        onClose={handleQueryResultsClose}
      />
    );
  };
            ...
            <div className="container">
              ...
              <SignedIn>
                ...
                <div className="nl-query-section">
                  <h3>Ask about your meters</h3>
                  <p>Type questions like "Show worst-performing meters this week" or "Which meters have high consumption?"</p>
                  <div className="nl-query-form">
                    <textarea
                      value={naturalLanguageQuery}
                      onChange={(e) => setNaturalLanguageQuery(e.target.value)}
                      placeholder="Ask about your meter performance, consumption, or costs..."
                      className="nl-query-textarea"
                      rows="3"
                    />
                    <button 
                      onClick={handleNaturalLanguageQuery}
                      className="nl-query-button"
